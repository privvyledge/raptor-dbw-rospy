...
"""
This script is used to control the speed of a New Eagle DBW enabled car using a joystick. It controls the actuators
using an internal DBW-ECM module controller (as opposed to actuator based percentage commands).
...
class JoystickSpeedControlNode:
    """docstring for ClassName"""

    def __init__(self, ):
        """Constructor for JoystickSpeedControlNode"""
        super(JoystickSpeedControlNode, self).__init__()
    ...
    def cmd_callback(self, event):
        ...
        # accelerator_pedal_msg.pedal_cmd = self.joystick_data['accelerator_pedal_joy'] * 100
        # for speed based acceleration
        accelerator_pedal_msg.speed_cmd = self.joystick_data['accelerator_pedal_joy']  # * 10 * 0.44704
    ...
    def joystick_callback(self, msg):
        ...
        self.joystick_data['accel_decel_limits'] = 3.0
        ...
        if msg.axes[7] != self.joy.axes[7]:
            if msg.axes[7] < -0.5:
                self.joystick_data['accelerator_pedal_joy'] -= 0.44704  # 0.44704 = 1 MPH
                if self.joystick_data['accelerator_pedal_joy'] < 0:
                    self.joystick_data['accelerator_pedal_joy'] = 0.0
            elif msg.axes[7] > 0.5:
                self.joystick_data['accelerator_pedal_joy'] += 0.44704
                if self.joystick_data['accelerator_pedal_joy'] > self.MAX_SPEED * self.SPEED_INCREMENT:
                    self.joystick_data['accelerator_pedal_joy'] = self.MAX_SPEED * self.SPEED_INCREMENT  # 4.4704
...
def main(args):
    ...
    optional_nodename = 'joystick_speed_control_node'
